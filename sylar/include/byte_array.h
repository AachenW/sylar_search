#ifndef __SYLAR_BYTE_ARRAY_H__
#define __SYLAR_BYTE_ARRAY_H__
...
public:
     ...
     * @return 返回实际数据的长度
     */
    uint64_t getReadBuffers(std::vector<iovec> &buffer, 
     ...
     * @return 返回实际数据的长度
     */
    uint64_t getReadBuffers(std::vector<iovec> &buffer, uint64_t len,
                            uint64_t position);
