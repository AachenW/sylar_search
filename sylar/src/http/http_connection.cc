...
HttpResult::HttpResult(int result_, 
...
HttpResult::ptr HttpConnection::DoRequest(HttpMethod method, 
    ...
    for (auto &header: headers) {
        if (0 == strcasecmp(header.first.c_str(), "connection")) {
            if (0 == strcasecmp(header.second.c_str(), "keep-alive")) {
                req->setClose(false);
            }
            continue;
        }

        if (!has_host && 0 == strcasecmp(header.first.c_str(), "host")) {
    ...
    if (client_parser.chunked) {
        ...
        do {
            do {
                ...
                len -= nparse;
                /// 读取了数据，但是待解析缓冲区已满（解析速度远小于读取速度） 
                if (len == (int)buff_size) {
                    close();
                    return nullptr;
                }
            } while (!parser->isFinished()); // 如果拿不到长度，则循环
            
            len -= 2;   // 去掉末尾的换行符，匹配http协议解析三方库
        ...
        } while (!client_parser.chunks_done);   // 如果还有chunk，则循环
...
HttpConnectionPool::ptr HttpConnectionPool::Create(const std::string &uri, 
    ...
    return std::make_shared<HttpConnectionPool>(turi->getHost(), virtualHost, 
            turi->getPort(), turi->getScheme() == "https", max_size, 
...
HttpConnection::ptr HttpConnectionPool::getConnection() {
    ...
    while (!m_conns.empty()) {
        ...
        m_conns.pop_front();
        if (!conn->isConnected()) {
            invalid_conns.emplace_back(conn);
            continue;
        }
...
HttpResult::ptr HttpConnectionPool::doRequest(HttpMethod method,
    ...
    if (!has_host) {
        if (m_virtualHost.empty()) {
            ...
            req->setHeader("Host", m_virtualHost);
        }
    }
    req->setBody(body);
