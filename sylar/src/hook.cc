...
/// @brief 异步io实现方式
/// @tparam OriginFun hook原函数类型
/// @tparam ...Args 可变参数模版
/// @param fd 文件句柄
/// @param fun hook原函数
/// @param hook_fun_name hook原函数名称
/// @param event IO事件
...
retry:
    ...
    if (n == -1 && errno == EAGAIN) {
        ...
        if (rt) {
            SYLAR_LOG_ERROR(g_logger) << hook_fun_name << " addEvent(" << fd 
                                      << ", " << event << ")";
            if (timer) {
                timer->cancel();
            }
            ...
            /// @brief 此处存在两种唤醒情况：1. 定时器超时会唤醒；2. IO读写事件发生会唤醒
            if (timer) {
                timer->cancel();
            }
            if (tinfo->cancelled) {
            ...
            goto retry; // 当被唤醒后需要重新读取数据（IO事件发生）
        }
    }
...
int connect_with_timeout(int sockfd, const struct sockaddr *addr, socklen_t addrlen, uint64_t timeout_ms) {
    ...
    if (rt == 0) {
        ...
        SYLAR_LOG_ERROR(g_logger) << "connect addEvent(" << sockfd 
                                  << ", WRITE) error";
    }
