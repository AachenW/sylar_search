...
bool Address::Lookup(std::vector<Address::ptr> &result, 
    ...
    if (!host.empty() && host[0] == '[') {
        // 从host的第二个字符开始的host.size() - 1个字节内查找字符‘]’
        const char *endipv6 = (const char *)memchr(host.c_str() + 1, ']', 
                                                   host.size() - 1);
        if (endipv6) {
            if (*(endipv6 + 1) == ':') {
                service = endipv6 + 2;
...
Address::ptr Address::LookpAny(const std::string &host, int family, 
                               int type, int protocol) {
...
bool Address::GetInterfaceAddress(
    ...
    if (iface.empty() || iface == "*") {
        if (family == AF_INET || family == AF_UNSPEC) {
            result.emplace_back(std::make_pair(Address::ptr(new IPv4Address()),
                                               0u));
        }
        if (family == AF_INET6 || family == AF_UNSPEC) {
            result.emplace_back(std::make_pair(Address::ptr(new IPv6Address()),
                                               0u));
        }
        return true;
    }
...
IPv4Address::IPv4Address(uint32_t address, uint16_t port) {
    ...
    // m_addr.sin_addr.s_addr = inet_addr("132.241.5.10")
